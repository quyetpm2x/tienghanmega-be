...
router.post('/',   protect, permit('tests.topikCreate'), async (req, res) => { success(res, await TopikQuestion.create(req.body), 'Tạo thành công', 201) });
router.put('/:id', protect, permit('tests.topikUpdate'), async (req, res) => {
  const doc = await TopikQuestion.findByIdAndUpdate(req.params.id, req.body, { new: true });
  success(res, doc, 'Cập nhật thành công');
});
router.delete('/:id', protect, permit('tests.topikDelete'), async (req, res) => {
