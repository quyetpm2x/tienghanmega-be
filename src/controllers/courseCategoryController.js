...
exports.create = async (req, res, next) => {
  ...
  if (!label?.trim()) return next(new AppError('Tên danh mục không được để trống', 400));
  ...
  const cat = await CourseCategory.create({ key, label: label.trim(), color: color || '#6366f1', bg, order: count });
...
exports.update = async (req, res, next) => {
  ...
  const cat = await CourseCategory.findByIdAndUpdate(
    req.params.id,
    { label: label.trim(), ...(color ? { color, bg } : {}) },
